...
class WP_Scripts {
	 ...
	 */
	public function do_items($handles = false, $group = 0) {
		/**
	 ...
	 */
	public function do_item($handle, $group = 0) {
		...
		echo sprintf("<script%s src='%s' id='%s-js'></script>\n", $this->type_attr, $obj->src, esc_attr($handle));
	 ...
	 * @return bool True on success, false on failure.
	 */
	private function determines_deps($handles, $recursion = false, $group = false) {
